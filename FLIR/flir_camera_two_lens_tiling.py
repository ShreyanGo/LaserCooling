...
def create_visualization():
    ...
    def draw_ray_diagram(ax, focal_length1, focal_length2, object_distance, lens_separation):
        ax.clear()
        ...
        try:
            ...
            # Calculate the final image distance after the second lens
            # If intermediate image is before the second lens, it's a real object
            # If intermediate image is after the second lens, it's a virtual object
            if second_object_distance > 0:
                # Real object for second lens
                final_image_distance = (second_object_distance * focal_length2) / (second_object_distance - focal_length2)
            else:
                # Virtual object for second lens (intermediate image after second lens)
                # For virtual objects, object distance is negative in the lens equation
                final_image_distance = (second_object_distance * focal_length2) / (second_object_distance - focal_length2)
            ...
            ax.text((mirror_pos + lens1_pos)/2, y_min * 0.8, lens_to_mirror_text, 
                    ha='center', va='bottom', fontsize=9, bbox=dict(facecolor='white', alpha=0.7))
            
            lens_separation_text = f"{lens_separation:.1f} inches\n({lens_separation_mm:.1f} mm)"
            ax.text((lens1_pos + lens2_pos)/2, y_min * 0.8, lens_separation_text, 
                    ha='center', va='bottom', fontsize=9, bbox=dict(facecolor='white', alpha=0.7))
            
            lens_to_sensor_text = f"{final_image_distance:.1f} mm"
            ax.text((lens2_pos + sensor_pos)/2, y_min * 0.8, lens_to_sensor_text, 
                    ha='center', va='bottom', fontsize=9, bbox=dict(facecolor='white', alpha=0.7))
    ...
    def draw_mirror_tiling(ax, focal_length1, focal_length2, object_distance, lens_separation):
        ax.clear()
        ...
        try:
            ...
            laser_radius_mm = 1.3  # mm on the mirror
            ...
            textstr = '\n'.join((
                ...
                f'Total magnification: {total_magnification:.4f}',
                f'Lens 1 magnification: {magnification1:.4f}',
                f'Lens 2 magnification: {magnification2:.4f}',
    ...
    def update_plots(focal_length1, focal_length2, object_distance, lens_separation):
        draw_ray_diagram(ax_rays, focal_length1, focal_length2, object_distance, lens_separation)
        draw_mirror_tiling(ax_mirror, focal_length1, focal_length2, object_distance, lens_separation)
        fig.canvas.draw_idle()
    
    # Initial plots
    update_plots(focal_length_init, second_focal_length_init, object_distance_init, lens_separation_init)
    
    # Create sliders - positioned below the plots
    ax_focal_length1 = plt.axes([0.2, 0.15, 0.65, 0.03], facecolor='lightgoldenrodyellow')
    ax_focal_length2 = plt.axes([0.2, 0.10, 0.65, 0.03], facecolor='lightgoldenrodyellow')
    ax_object_distance = plt.axes([0.2, 0.05, 0.65, 0.03], facecolor='lightgoldenrodyellow')
    ax_lens_separation = plt.axes([0.2, 0.20, 0.65, 0.03], facecolor='lightgoldenrodyellow')
    ...
    # Create reset button
    reset_ax = plt.axes([0.01, 0.10, 0.05, 0.04])
    ...
    def update(val):
        ...
        update_plots(focal_length1, focal_length2, object_distance, lens_separation)
